...
class TrainConfig:
    ...
    # When set, overrides `steps`: see `resolve_train_config`'s world-size-aware
    # `ceil(target_tokens / (batch_size * context_length * world_size))` derivation.
    # Ported from the sibling custom-gpt-50m-ddp project specifically so a multi-node
    # DDP run can be sized in tokens (the thing you actually have a budget for) instead
    # of hand-computing `steps` and forgetting to divide by `world_size` — see
    # docs/GPU_TRAINING.md's "Multi-Node DDP on a Single AZ" section.
    target_tokens: int | None = None
    # "ddp" (replicate the full model on every rank, sync gradients) or "fsdp" (shard
    # parameters/gradients/optimizer state across ranks, all-gather per layer as
    # needed). Irrelevant at world_size=1 — both collapse to the same plain,
    # single-process path. See docs/DISTRIBUTED_TRAINING.md for the mechanism
    # difference and why this project supports both rather than picking one.
    parallelism: str = "ddp"
...
_TRAIN_ENV_OVERRIDES = {
    ...
    "parallelism": ("GPT_PARALLELISM", str),
}
...
def resolve_train_config(context_length=None, world_size=1):
    ...
    If `GPT_TARGET_TOKENS` is set, it takes precedence over `GPT_STEPS`: the latter is
    derived as ``ceil(target_tokens / (batch_size * context_length * world_size))``.
    Passing `world_size` here (from `cli/train.py`'s `WORLD_SIZE` env var) is what
    keeps a token target correct under DDP — each rank still only processes its own
    `batch_size * context_length` tokens per step, so the *global* tokens/step scale
    with `world_size`, and a token target that ignores that silently trains for
    `world_size` times fewer tokens than intended once launched under `torchrun`.
    """
    overrides = {}
    ...
    if cfg.parallelism not in ("ddp", "fsdp"):
        raise ValueError(
            f"Unknown GPT_PARALLELISM {cfg.parallelism!r}. Use 'ddp' or 'fsdp'."
        )
    ...
    ctx = context_length or ModelConfig().context_length
    tokens_per_step = cfg.batch_size * ctx * world_size
...
def load_settings(preset_name=None, world_size=1):
    ...
    train_cfg = resolve_train_config(model_cfg.context_length, world_size=world_size)
    return model_cfg, train_cfg, Paths(label=label), label
