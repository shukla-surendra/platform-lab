...
- One Compute Engine VM (`g2-standard-24` = **2x** NVIDIA L4 24GB each, bf16-capable —
  this is the multi-GPU sibling of `infra/gcp-gpu-node/`'s single-GPU `g2-standard-4`;
  see this module's own `docs/training_sop.md` for why it's a separate module rather
  than just a bigger machine_type on the original: multi-process (`torchrun`) launch
  orchestration differs, the original never launches training at all)
