...
A fork of [`custom-gpt-350m`](../custom-gpt-350m/README.md), hardened specifically for
**multi-node distributed training on AWS** — DDP first and foremost (verified with a
real 2-rank smoke test, `scripts/ddp_smoke_test.py`, against this project's actual
corpus, not a placeholder), with an FSDP path also implemented and verified
(`scripts/fsdp_smoke_test.py`, including a real save/resume round trip — see
[`docs/DISTRIBUTED_TRAINING.md`](docs/DISTRIBUTED_TRAINING.md)) for when that's the
one being exercised instead. Token budgeting is world-size-aware
(`GPT_TARGET_TOKENS`, correct under `torchrun --nnodes=2`, for either mode), and
[`docs/GPU_TRAINING.md`](docs/GPU_TRAINING.md)'s "Multi-Node DDP on a Single AZ"
section covers the actual AWS setup, launch commands, and cost (compute + network).
Everything else — architecture, single-GPU training — is identical to the sibling
project this was forked from; the data pipeline differs only in that this fork
already has its corpus tokenized (see "Run order" below).

**This fork's corpus, as it stands**: `data/train.bin`/`test.bin` (books +
cosmopedia, 1,015,850,483 train tokens) and `tokenizer/tokenizer.json` are already
built and in place — not a placeholder, the real thing this project trains on right
now. `GPT_TARGET_TOKENS=4000000000` (the infra default) is sized against this exact
corpus (~3.94 epochs over it), not the model's theoretical Chinchilla-optimal ~6.95B —
see `docs/GPU_TRAINING.md`'s "Sizing the run" section for why.
...
## Run order — the tokenizer comes first

The embedding table is sized to this project's own vocabulary, so the tokenizer is a
prerequisite rather than an option:

```bash
make tokenizer    # 1. train the 32K BPE on data/train.txt
make tokenize     # 2. build data/*.bin with it
make train        # 3. train
```

**In this fork, right now, steps 1-2 are already done** — `tokenizer/tokenizer.json`
and `data/train.bin`/`test.bin` exist (copied/tokenized from the real corpus, not a
placeholder). `make train` (single-process) or a `torchrun` DDP/FSDP launch (see
`docs/GPU_TRAINING.md`) can be run directly. There's no `data/train.txt` in this
fork — only the already-tokenized `.bin` files — so `make tokenizer`/`make tokenize`
would need the raw text staged first if ever re-run from scratch here.

`tokenize` and `train` both fail immediately with an explanation if the tokenizer is
missing. Retraining the tokenizer invalidates every `.bin` (rebuilt automatically) and
every checkpoint (not migratable). Corpus layout and the cross-tokenizer safety guard:
...
Part of [llm-engineering](../../README.md)'s **from-scratch track** — see the
