...
A hands-on lab for exploring small language models several ways: **training a GPT-style
model completely from scratch**, **fine-tuning an existing pretrained model**,
**serving an original author checkpoint standalone**, unmodified, as its own baseline,
**post-training quantization** of an already-trained model, and **local inference**
against models already pulled/running on this machine (Ollama), not trained here at all.
These tracks are meant to grow over time as more experiments get added — this isn't one
fixed project, it's a home for many small, runnable LLM experiments.
...
Each experiment lives in its own subfolder under one of these tracks, with its own
