...
- **2026-09-04, later same day** — Steps 2-4 of the plan above actually happened, but as real hands-on work in `personal_assistant/` (a separate project, not this repo) rather than as a written module here — matches the plan's own "hands-on, no prose doc" call for AKS. `az` CLI installed, `az login` succeeded against a real single-tenant/single-subscription personal account ("Default Directory" / "Azure subscription 1"), confirming the tenant↔subscription relationship from the Entra ID module live rather than abstractly. Terraform written (not yet applied) for AKS (Free SKU tier, 2x Standard_B2s, kubenet) + ACR + an `AcrPull` role assignment on the AKS kubelet's Managed Identity — a live instance of the Entra ID module's §3c Managed-Identity pattern, contrasted against this app's own dead AWS boto3 code (which relies on boto3's default credential chain, the equivalent AWS pattern) in a new companion doc: `personal_assistant/docs/AWS_vs_AZURE_PERMISSIONS.md`. The one piece that genuinely belongs in *this* repo rather than the app's own: **`azure/cheatsheets/az-cli.md`** (new — this track's `cheatsheets/` folder existed only in the planned layout diagram until now), a cross-cutting `az` CLI reference (auth, resource groups, RBAC, VNet, Blob Storage, AKS, ACR, cost, global flags) with AWS CLI equivalents noted, terse "one-page recall" style matching `aws/cheatsheets/vpc.md`'s existing convention. VNet/Entra ID M1 gates from the plan above are still open/unanswered — this work ran ahead of clearing them, learner's explicit direction each time ("I will not proceed without fully understanding" was about the AWS/Azure permissions contrast specifically, answered via the new doc's Q&A, not the formal M1 gate questions).
- **2026-09-04, still later** — Real AKS deployment completed (Terraform applied for real, not just written): container registry → cluster → app, verified live via `curl` through the public Load Balancer IP. Extended item 4's Managed-Identity tie-back one step further: added a **fourth Terraform stage** (`personal_assistant/terraform/keyvault/`) putting the app's `OPENAI_API_KEY` in a real Azure Key Vault (RBAC-authorization mode), fetched by the backend pod via **Workload Identity** (a dedicated `azurerm_user_assigned_identity` federated to the pod's own Kubernetes ServiceAccount token through AKS's OIDC issuer — the same federated-credential shape already used for GitHub Actions in `terraform/cicd/`, just trusting a K8s ServiceAccount subject instead of a GitHub repo/branch one) plus the Secrets Store CSI Driver (an AKS-managed addon, `key_vault_secrets_provider` on the cluster resource). Live-verified end-to-end: signup → login → create chat → real OpenAI completion, through the public LB, with the key never touching Terraform state, `.tfvars`, or a manually-managed K8s Secret at any point — only a one-time manual `az keyvault secret set`. Hit one real failure mode worth remembering: applying before the vault secret existed left the new pod stuck in `ContainerCreating` on a clean `404 SecretNotFound` (not a 403 — proof the identity/RBAC pipeline itself was already correct) while the *old* pod kept serving traffic the whole time, self-healing within ~20s of the secret being set with no re-apply needed. Full narrative, including the exact Terraform diffs and troubleshooting, logged in `personal_assistant/docs/AWS_vs_AZURE_PERMISSIONS.md`.
