#!/usr/bin/env python3
"""Chunk, embed, and load local .md/.txt documents into the Qdrant collection.
...
def ingest(folder: Path) -> None:
    ...
    client = get_client()
    store = QdrantVectorStore.create_collection(client, config.COLLECTION_NAME, config.EMBEDDING_DIM)
    ...
    for source, text in documents:
        ...
        payloads = [{"source": source, "chunk_index": idx, "text": chunk} for idx, chunk in enumerate(chunks)]
