...
Local LLM/vision-model experimentation — everything here runs against models
already pulled/running on this machine (mainly via [Ollama](https://ollama.com)),
not a hosted API. `uv`-managed (`pyproject.toml`/`uv.lock`, Python 3.12+);
`ollama-chatbox/` is its own separate npm project.
