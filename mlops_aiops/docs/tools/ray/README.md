...
| **Databricks** | Not actually a Ray-alternative at the same layer — Databricks is a *managed platform* (cluster provisioning/autoscaling, notebooks, Unity Catalog, Delta Lake, MLflow, job scheduling) wrapped around Spark as its compute engine. Ray Core by itself is only the compute-engine layer, with none of that platform tooling built in. The platform-layer comparison that's actually apples-to-apples is **Anyscale** (Ray's own commercial company) vs. Databricks — Anyscale is to Ray what Databricks is to Spark. Self-hosted, [KubeRay](../kuberay/README.md) is the equivalent of Databricks' cluster-provisioning layer, minus the notebook/governance/catalog tooling. |
